...
class InstanceManagerService {
  ...
  Future<bool> actionsHandler(
      String url, String instanceName, List<Map<String, dynamic>> actions, Function(String) updateStatusMessage)
  async {
    ...
    if((instance["state"] == 'stopped' && actions[0]['action'] == 'turn-off')
        || (instance["state"] == 'running' && actions[0]['action'].contains('create'))) {
            ...
            if (action.containsKey("pass_condition")) {
              ...
              for (int attempt = 1; attempt <= action["pass_condition"]['attempts']; attempt++) {
                ...
                if (attempt < action["pass_condition"]['attempts']) {
                  await Future.delayed(Duration(seconds: action["pass_condition"]['timeout']));
                }
              }
  ...
  Future<Map<String, dynamic>> actionResolver(String url, String instance, String action, String? resourceId, String? conditionVal) async {
